  ...
  const astInterpreters = {
    ...
    CallExpression(node, scope) {
      ...
      if (node.callee.type === 'MemberExpression') {
        ...
        const fn = scope.get(evaluate(node.callee));
        return fn.apply(null, args);
      }
    },
    MemberExpression(node, scope) {
      const objectName = evaluate(node.object);
      const propertyName = evaluate(node.property);
      return scope.get(objectName)[propertyName];
    },
    NumericLiteral(node) {
      return node.value;
    },
    Identifier(node, scope) {
      return scope ? scope.get(node.name) : node.name;
    }
  };
  ...
  function evaluate(node, scope) {
    try {
      ...
      if (e && e.message && e.message.indexOf('astInterpreters[node.type] is not a function') != -1) {
        errorMsg = 'unsupported ast type: ' + node.type;
      } else {
        errorMsg = e.message;              
      }
